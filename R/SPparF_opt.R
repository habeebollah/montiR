...
#'
#' @description
#' Function used in the maximum likelihood minimization to estimate Fox's biomass dynamic model parameters using lognormal distribution. The use of maximum likelihood is important to remove potential error during data collection.
...
#'              hessian=TRUE)
#'
#' SPparF_val <- data.frame(SPpar = c("K", "B0", "r", "q", "sigma"),
#'                          init_pars = c(K, B0, r, q, 0.1),
#'                          fitted_pars = exp(fit$par))
#'
SPparF_opt <- function(inpars, df, OWT=FALSE, Frate = 0.7, weight = 100){
  K <- exp(inpars[1])
  B0 <- exp(inpars[2])
  r <- exp(inpars[3])
  q <- exp(inpars[4])
