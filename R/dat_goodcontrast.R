#' @title Good contrast data with process and catch error
#'
#' @description A data set containing year, catch, effort and biomass which shows CPUE in good contrast.
#' This data shows a stochasticity using lognormal process and catch error.
#'
#' @format A data frame with 20 rows and 3 variables:
#' \describe{
#'   \item{year}{year when fishing activity occured.}
#'   \item{catch}{catch per annum in MT.}
#'   \item{effort}{effort unit per annum.}
#'   \item{biomass}{fish biomass per annum in MT.}
#' }
#'
#' @references Magnusson, Arni, and Ray Hilborn. "What makes fisheries data informative?." Fish and Fisheries 8, no. 4 (2007): 337-358.
#'
"df.goodcontrast"
