...
#'
#' @param inpars surplus production parameters which consist of K (carrying capacity), B0 (biomass when fishing is started), r (growth parameter), q (catchability coefficient)
...
#'
RPcalc <- function(inpars, SPmodel=Schaefer){
  K <- inpars[1]
  B0 <- inpars[2]
  r <- inpars[3]
  q <- inpars[4]
